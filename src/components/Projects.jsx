  ...
  return (
    <div className="container-projects">
      <div className="projects-title">
        ...
        <p className="text-white text-lg md:text-2xl mt-1">return<span className="text-blue-500">(</span>projects<span className="text-yellow-500">()</span><span className="text-blue-500">)</span>;</p>
      </div>
      <div className="flex justify-center w-full relative mt-5 gap-3 md:gap-6">
        <p>{"<"}</p>
        <a className="btn btn-xs nav-btn-projects" href="#item1" onClick={(e) => {
            e.preventDefault();
            carouselRef.current.scrollTo({ left: carouselRef.current.clientWidth, top: 0, behavior: 'smooth' });
        }}>1</a>
        <a className="btn btn-xs nav-btn-projects" href="#item2" onClick={(e) => {
            e.preventDefault();
            carouselRef.current.scrollTo({ left: carouselRef.current.clientWidth * 2, top: 0, behavior: 'smooth' });
        }}>2</a>
        <a className="btn btn-xs nav-btn-projects" href="#item3" onClick={(e) => {
            e.preventDefault();
            carouselRef.current.scrollTo({ left: carouselRef.current.clientWidth * 3, top: 0, behavior: 'smooth' });
        }}>3</a>
        <a className="btn btn-xs nav-btn-projects" href="#item4" onClick={(e) => {
            e.preventDefault();
            carouselRef.current.scrollTo({ left: carouselRef.current.clientWidth * 4, top: 0, behavior: 'smooth' });
        }}>4</a>
        <a className="btn btn-xs nav-btn-projects" href="#item5" onClick={(e) => {
            e.preventDefault();
            carouselRef.current.scrollTo({ left: carouselRef.current.clientWidth * 5, top: 0, behavior: 'smooth' });
        }}>5</a>
        <a className="btn btn-xs nav-btn-projects" href="#item6" onClick={(e) => {
            e.preventDefault();
            carouselRef.current.scrollTo({ left: carouselRef.current.clientWidth * 6, top: 0, behavior: 'smooth' });
        }}>6</a>
        <a className="btn btn-xs nav-btn-projects" href="#item7" onClick={(e) => {
            e.preventDefault();
            carouselRef.current.scrollTo({ left: carouselRef.current.clientWidth * 7, top: 0, behavior: 'smooth' });
        }}>7</a>
        <a className="btn btn-xs nav-btn-projects" href="#item8" onClick={(e) => {
            e.preventDefault();
            carouselRef.current.scrollTo({ left: carouselRef.current.clientWidth * 7, top: 0, behavior: 'smooth' });
        }}>8</a>
        <p>{">"}</p>
        ...
        <div ref={carouselRef} className="carousel carousel-center h-custom p-4 space-x-12 bg-stone-600/20 rounded-box mt-9 py-10 px-14">
          <div className="carousel-item shadow-custom">
            <div className="indicator">
              <div style={{width:"75vw"}} className="carousel-intro-div rounded-box bg-stone-800">
                  ...
                  <div ref={scrollerMasterRef} className='scroller-master'>
                    <div className="scroller-wheel">
                      <div className='flex flex-col gap-2'>
                        ...
                        <p className='rotate-90 text-2xl font-thin separator-line'>/</p>
          ...
          <div id="item4" className="carousel-item shadow-custom">
            <div className="indicator">
              <span className="indicator-item indicator-start badge badge-primary ml-14 p-4 text-lg font-bold">Alta.ai</span>
              <span
                onClick={(e) => {
                  e.preventDefault();
                  handleViewClick('AltaAi');
                }}
                className="transform hover:scale-110 hover:bg-stone-500 cursor-pointer indicator-item indicator-middle indicator-center badge badge-primary mt-custom p-4 text-lg font-bold"
              >
                View
              </span>
              <video src="https://res.cloudinary.com/djr22sgp3/video/upload/v1682352612/Alta.ai_showreel_wvz4sx.mp4" style={{width:"75vw"}} className="rounded-box scale-effect object-cover" autoPlay loop muted playsInline webkit-playsinline="true" />
            </div>
          </div>
          ...
          <div id="item8" className="carousel-item shadow-custom">
            <div className="indicator">
